...
class DashboardController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $highPriority = Concern::where('barangay_id', $barangayId)
        ...
        $stats = [
            ...
            'by_severity' => [
                ...
                'medium' => Concern::where('barangay_id', $barangayId)->where('severity', 'medium')->count(),
                'low' => Concern::where('barangay_id', $barangayId)->where('severity', 'low')->count(),
            ],
        ];
        ...
        $incidents = Mission::with(['concern', 'proof.media'])
            ...
            ->map(function ($mission) {
                ...
                $severity = $mission->concern->severity ?? 'medium';
                $missionStatus = $mission->status instanceof MissionStatus ? $mission->status->value : $mission->status;
                ...
                return [
                    ...
                    'type_icon' => MapHelpers::typeIconFromText($mission->concern->title ?? ''),
                    'location' => $mission->concern->address_text ?? 'Pinned Location',
                    'ai_severity' => MapHelpers::scoreFromSeverity($severity),
                    'priority' => MapHelpers::priorityFromSeverity($severity),
                    'status' => MapHelpers::queueStatusFromMission($missionStatus),
        ...
        $activities = DB::table('audit_logs')
            ...
            ->map(function ($log) {
                return [
                    ...
                    'time' => $log->created_at ? \Carbon\Carbon::parse($log->created_at)->diffForHumans() : 'Recently',
                    'icon' => MapHelpers::activityIcon($log->action),
                ];
            })->toArray();
